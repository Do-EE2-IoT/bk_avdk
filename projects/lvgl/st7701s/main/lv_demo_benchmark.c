...
static void benchmark_init(void)
{
    ...
    /* create touch indicator timer */
    touch_dot = NULL;
    touch_timer = lv_timer_create(touch_timer_cb, 20, NULL);
}
...
static void touch_timer_cb(lv_timer_t * timer)
{
    ...
    const lv_coord_t dot_size = 14;

    if(state == LV_INDEV_STATE_PR) {
        if(touch_dot == NULL) {
            touch_dot = lv_obj_create(lv_scr_act());
            lv_obj_set_size(touch_dot, dot_size, dot_size);
        ...
        lv_obj_set_pos(touch_dot, p.x - dot_size / 2, p.y - dot_size / 2);
