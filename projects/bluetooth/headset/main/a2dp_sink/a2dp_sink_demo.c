...
void bt_audio_sink_demo_main(void *arg)
{
    ...
    else
    {
        ...
        os_printf("initial volume %d %02x:%02x:%02x:%02x:%02x:%02x\r\n", s_a2dp_vol,
                  recon_addr[5],
                  recon_addr[4],
                  recon_addr[3],
                  recon_addr[2],
        ...
        if (kNoErr == err)
        {
            ...
            case BT_AUDIO_D2DP_START_MSG:
            {
                ...
                if (CODEC_AUDIO_SBC == p_codec_info->type)
                {
                    ...
                    uint32_t sample_rate = p_codec_info->cie.sbc_codec.sample_rate;
                    ...
                    os_printf("cm:%d, c:%d, s:%d, b:%d, bi:%d, frame_length:%d , sample rate = %d\n", chnl_mode, chnls, subbands, blocks, bitpool, frame_length, sample_rate);
                ...
                else if (CODEC_AUDIO_AAC == p_codec_info->type)
                {
                    ...
                    {
                        os_printf("AAC Decoder init successfully");
                    }
...
int a2dp_sink_demo_init(uint8_t aac_supported)
{
    ...
    {
#if (!CONFIG_AAC_DECODER)
        os_printf("%s AAC is not supported!\n", __func__);
        return -1;
#else
        os_printf("%s AAC is supported!\n", __func__);
#endif
    }
    ...
    os_printf("Init bt_audio_sink_demo_task_init\r\n");
...
{
    bk_err_t ret = 0;
    uint32_t sample_rate = (bt_audio_a2dp_sink_codec.type == CODEC_AUDIO_SBC ? bt_audio_a2dp_sink_codec.cie.sbc_codec.sample_rate : bt_audio_a2dp_sink_codec.cie.aac_codec.sample_rate);
        ...
        while (frame_nodes)
        {
            ...
            for (; i < s_frame; i++)
            {
                ...
                if (CODEC_AUDIO_SBC == bt_audio_a2dp_sink_codec.type)
                {
                    ...
                    ret = audio_play_pcm_i2s_write((uint8_t *)dst, w_len, 200);
                    if (ret != BK_OK)
                    {
                        LOGE("pcm_i2s_write fail: %d \n", ret);
                        break;
                    }
                    else
                    {
    ...
    ret = audio_play_pcm_i2s_stop();
    if (ret != BK_OK)
    {
        LOGE("%s stop pcm i2s err %d\n", __func__, ret);
    }
