...
{
    uint8_t dac_gain = (0x7f / 15.0) * hfp_vol;
...
static uint32_t hfp_expand_mono_to_stereo(uint8_t *dst, const uint8_t *src, uint32_t src_len)
{
    uint32_t samples = src_len / 2;
...
{
    bk_err_t ret = 0;
    uint32_t sample_rate = ((CODEC_VOICE_MSBC == bt_audio_hfp_hf_codec) ? 16000 : 8000);
    LOGI("%s wait a2dp task end\n", __func__);
    extern int32_t wait_a2dp_speaker_task_end(void);
    wait_a2dp_speaker_task_end();

    bk_bt_dac_set_gain(9);
    ...
    while (hf_auido_start)
    {
...
#if HF_LOCAL_ROLLBACK_TEST
        (void)(packet_len);
        uint32_t stereo_len = hfp_expand_mono_to_stereo(stereo_tmp, hf_speaker_buffer, mic_read_size);
        int size = audio_play_pcm_i2s_write(stereo_tmp, stereo_len, 200);
#else
        ...
        while (ring_buffer_particle_len(&s_hfp_sco_spk_data_rb) >= 100)
        {
            ...
            write_len = hfp_expand_mono_to_stereo(stereo_tmp, tmp_recv, write_len);
            size = audio_play_pcm_i2s_write(stereo_tmp, write_len, 200);

            if (size != BK_OK)
            {
                LOGE("audio_play_pcm_i2s_write err: %d %d!!!\n", size, write_len);
                break;
            }
            else
            {
