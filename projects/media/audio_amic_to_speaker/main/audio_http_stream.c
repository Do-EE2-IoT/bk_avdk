...
bk_err_t audio_http_stream_start(uint32_t sample_rate, uint32_t channels)
{
	...
	ret = rtos_init_queue(&s_free_queue, "aud_http_free", sizeof(audio_http_msg_t), AUDIO_HTTP_QUEUE_DEPTH);
	if (ret != BK_OK)
		return ret;

	ret = rtos_init_queue(&s_send_queue, "aud_http_send", sizeof(audio_http_msg_t), AUDIO_HTTP_QUEUE_DEPTH);
	if (ret != BK_OK) {
